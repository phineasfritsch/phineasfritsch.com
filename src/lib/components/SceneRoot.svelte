<script lang="ts">
	...
	function surfQuat(lat: number, lon: number) {
		const φ = (lat * Math.PI) / 180;
		const λ = (lon * Math.PI) / 180;
		const n = new THREE.Vector3(Math.cos(φ) * Math.cos(λ), Math.sin(φ), Math.cos(φ) * Math.sin(λ)).normalize();
	...
	const treeMat2 = new THREE.MeshToonMaterial({ color: '#43A047', flatShading: true });
	...
	const trees: Array<{ lat: number; lon: number; s: number }> = [
		...
		{ lat: 25, lon: 60, s: 1.1 }, { lat: 20, lon: 55, s: 0.9 }, { lat: 28, lon: 66, s: 1.0 },
		{ lat: 18, lon: 70, s: 0.85 }, { lat: 24, lon: 72, s: 1.0 }, { lat: 30, lon: 62, s: 0.9 },
		// NW continent (FratHouse)
		{ lat: 20, lon: 118, s: 1.0 }, { lat: 24, lon: 112, s: 1.1 }, { lat: 16, lon: 124, s: 0.9 },
		{ lat: 28, lon: 116, s: 1.0 }, { lat: 22, lon: 108, s: 0.85 }, { lat: 18, lon: 122, s: 1.0 },
		// Southern continent
		{ lat: -12, lon: 86, s: 1.0 }, { lat: -16, lon: 94, s: 0.9 }, { lat: -10, lon: 96, s: 1.0 },
		{ lat: -18, lon: 88, s: 0.85 }, { lat: -14, lon: 80, s: 1.1 },
		// Central island cluster
		{ lat: 6, lon: 78, s: 0.8 }, { lat: 4, lon: 75, s: 0.9 },

		// ── Right side lon 130–190 ─────────────────────────────────────────
		{ lat: 15, lon: 148, s: 1.0 }, { lat: 20, lon: 155, s: 0.9 }, { lat: 10, lon: 152, s: 1.1 },
		{ lat: -8, lon: 160, s: 0.85 }, { lat: -14, lon: 145, s: 1.0 },
		{ lat: 28, lon: 140, s: 0.9 }, { lat: 5, lon: 168, s: 1.0 },

		// ── Back face lon 190–250 ──────────────────────────────────────────
		{ lat: 18, lon: 210, s: 1.0 }, { lat: 10, lon: 220, s: 0.9 }, { lat: -5, lon: 205, s: 1.1 },
		{ lat: 25, lon: 230, s: 0.9 }, { lat: -15, lon: 225, s: 1.0 },

		// ── Left side lon 250–320 ──────────────────────────────────────────
		{ lat: 12, lon: 268, s: 1.0 }, { lat: 22, lon: 275, s: 0.9 }, { lat: -10, lon: 260, s: 1.0 },
		{ lat: 5, lon: 290, s: 0.85 }, { lat: -20, lon: 278, s: 1.1 }, { lat: 30, lon: 285, s: 0.9 },

		// ── Northern cap bits ──────────────────────────────────────────────
		{ lat: 35, lon: 90, s: 0.75 }, { lat: 38, lon: 200, s: 0.7 },
	];
</script>
...
<T.Group bind:ref={globeGroup}>
	...
	<!-- Southern continent -->
	<Land lat={-13} lon={90} radius={1.05} sides={8} color="#3a8c35" height={0.30} />
	<Land lat={-22} lon={83} radius={0.42} sides={6} color="#4caf50" height={0.20} />
	<Land lat={-20} lon={98} radius={0.40} sides={5} color="#4caf50" height={0.18} />
	<!-- Central island -->
	...
	<!-- ── RIGHT SIDE (lon 130–190) ─────────────────────────────── -->
	<Land lat={16} lon={150} radius={0.90} sides={8} color="#3a8c35" height={0.28} />
	...
	<Land lat={28} lon={138} radius={0.40} sides={5} color="#5aba50" height={0.18} />
	<Land lat={-2} lon={170} radius={0.38} sides={5} color="#4caf50" height={0.18} />

	<!-- ── BACK FACE (lon 195–240) ──────────────────────────────── -->
	<Land lat={18} lon={210} radius={0.80} sides={8} color="#3a8c35" height={0.26} />
	<Land lat={8}  lon={222} radius={0.50} sides={6} color="#4caf50" height={0.22} />
	...
	<Land lat={-18} lon={228} radius={0.42} sides={6} color="#4caf50" height={0.20} />
	...
	<Land lat={4}  lon={292} radius={0.40} sides={5} color="#5aba50" height={0.18} />
	<Land lat={-22} lon={280} radius={0.44} sides={6} color="#4caf50" height={0.20} />
	...
	{#each trees as { lat, lon, s }}
		...
		<T.Group position={[p.x, p.y, p.z]} quaternion={[q.x, q.y, q.z, q.w]}>
			...
			<T.Mesh position={[0, 0.30 * s, 0]}>
				<T.SphereGeometry args={[0.20 * s, 6, 5]} />
				<T is={treeMat} />
			</T.Mesh>
